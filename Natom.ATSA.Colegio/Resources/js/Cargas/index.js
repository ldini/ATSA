$(document).ready(function () {
    ...
    var oTable = $('#tbl').dataTable({
        ...
        "bServerSide": true,
        "ajax": {
            "url": "/colegiotest/cargas/ObtenerListadoIndex",
            "type": "POST"
        },
        "bLengthChange": true,
        "bFilter": true,
        ...
        "aoColumns": [
            ...
            { "bSortable": true, "sWidth": '17%' },
            {
                ...
                mRender: function (data, type, row) {
                    ...
                    var eliminado = row[8];
                    if (eliminado) {
                        html += '<a class="btn btn-danger btn-xs btn-ver-elimino" id="' + data + '">Eliminado (+info)</a>';
                    }
                    else {
                        html += '<a class="btn btn-danger btn-xs btn-eliminar" id="' + data + '" ' + (puede ? "puede" : "") + '>Eliminar</a>';
                    }
                    return html;
                }
            }
    ...
    $("#tbl").on("click", ".btn-ver-elimino", function (e) {
        ...
        $.ajax({
            type: "POST",
            url: '/colegiotest/cargas/ObtenerInfoAnulacion',
            data: JSON.stringify({
                cargaId: id
            }),
            dataType: "json",
    ...
    function Eliminar(id, motivo) {
        $.ajax({
            type: "POST",
            url: '/colegiotest/cargas/EliminarCarga',
            data: JSON.stringify({
                CargaId: id,
                motivo: motivo
            }),
